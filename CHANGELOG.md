...
### Changed
- **Build Artifacts:** Switched from a single Universal Binary to two separate application bundles: one for Intel (`CriptoMenu_Intel.app`) and one for Apple Silicon (`CriptoMenu_AppleSilicon.app`). This reduces the file size for each user.
